...
- **Authentication**: `Microsoft.Identity.Web`'s `AddMicrosoftIdentityWebApi` validates Azure AD (Entra ID) bearer tokens, configured from the `AzureAd` section in `appsettings.json` (`TenantId: "common"` — accepts work/school Azure AD accounts from any org plus personal Microsoft accounts). A global `AuthorizeFilter`, registered via `AddControllers(options => options.Filters.Add(new AuthorizeFilter()))`, protects every controller action by default (including future ones) — no per-controller `[Authorize]` attributes are used; a genuinely public endpoint would opt out with `[AllowAnonymous]`. `AzureAd:ClientId`/`Audience` are still placeholder GUIDs — no real Azure App Registration exists yet, so full token validation isn't testable end-to-end until the user creates one and supplies real values. **CORS**: a named policy (`BudganCorsPolicy`) sources allowed origins from an `AllowedOrigins` config array — empty (fail-closed) in base `appsettings.json`, `http://localhost:4200` allowed in `appsettings.Development.json`. Middleware order in `Program.cs`: `UseCors` → `UseAuthentication` → `UseAuthorization` → `MapControllers`. The Angular frontend (`web/`) has no MSAL/Azure AD client integration yet, so it will receive `401` on every API call until that follow-up work lands.
...
- Authentication/authorization/CORS are configured (see BudganSvr section above), but `AzureAd:ClientId`/`Audience` are placeholders pending a real Azure App Registration, and the Angular frontend has no client-side MSAL integration yet.
