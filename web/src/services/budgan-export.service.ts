...
export type AccountExportPayload = AllDataExportPayload;
...
export interface BudganExportService {
  ...
  ): Promise<Result<AllDataExportPayload>>;
...
export const BUDGAN_EXPORT_SERVICE = new InjectionToken<BudganExportService>(
  'BudganExportService',
);
  ...
  async pickSaveFile(
    ...
    try {
      return await showSaveFilePicker({
        ...
        types: [
          {
            description: 'Budgan files',
            accept: { 'application/octet-stream': ['.bdg'] },
          },
        ],
      });
    } catch {
